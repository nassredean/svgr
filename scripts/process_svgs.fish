...
while test $group_counter -le $num_groups
    ...
    set output_file (string join "" $output_directory $group_counter ".svg")
    eval "./bin/svgr arrange:grid $trimmed_files 3 1 -s 3 -t 8 | ./bin/svgr document:resize 228.6 279.4 | ./bin/svgr document:recolor \"#000000\" > $output_file"
